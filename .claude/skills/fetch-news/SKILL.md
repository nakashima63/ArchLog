...
- 同一URLは重複排除する
- **過去のニュースファイルとの重複禁止**: Markdownファイル生成前に `docs/news/` 配下の既存ニュースファイル（直近3件程度）を読み込み、同一URLの記事が既に掲載されていないかチェックする。同一URLまたは同一内容の記事（異なるURLでも同じ記事のクロスポスト等）は掲載しない。connpassイベントも同様に重複チェックする
- カテゴリごとにグループ化する
- 該当記事がないカテゴリは省略する
